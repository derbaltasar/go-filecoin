...
func TestMinerSubmitPoSt(t *testing.T) {
	...
	lastSubmission := secondProvingPeriodStart + LargestSectorSizeProvingPeriodBlocks + LargestSectorGenerationAttackThresholdBlocks
	...
	t.Run("after generation attack grace period rejected", func(t *testing.T) {
		...
		res, err = th.CreateAndApplyTestMessage(t, st, vms, minerAddr, 0, lastSubmission+1, "submitPoSt", ancestors, []types.PoStProof{proof})
		assert.NoError(t, err)
	...
	t.Run("late submission charged fee", func(t *testing.T) {
		...
		res, err = th.CreateAndApplyTestMessage(t, st, vms, minerAddr, 0, lastSubmission, "submitPoSt", ancestors, []types.PoStProof{proof})
		assert.NoError(t, err)
		assert.Error(t, res.ExecutionError)

		// Accepted on the deadline with a fee
		res, err = th.CreateAndApplyTestMessage(t, st, vms, minerAddr, 1, lastSubmission, "submitPoSt", ancestors, []types.PoStProof{proof})
		assert.NoError(t, err)
		assert.NoError(t, res.ExecutionError)
		assert.Equal(t, uint8(0), res.Receipt.ExitCode)
		...
		fee := LatePoStFee(postedCollateral, bh(secondProvingPeriodStart+LargestSectorSizeProvingPeriodBlocks), bh(lastSubmission), bh(LargestSectorGenerationAttackThresholdBlocks))
		miner := state.MustGetActor(st, minerAddr)
